function isLoadMapSuc() {
	return typeof GFRAME != "undefined" && GFRAME != null && GFRAME.isInitSuc;
	...
	if (mapmarker != null) {
		...
		if (mapmarker.nameMarker == null) {
			if (typeId == 1) {
				...
				if(!iconImage) {
					iconImage = "marker/1.gif"; 
				}else {
					iconImage = "marker/"+iconImage;
				}
				var icon = new BMap.Icon(GFRAME.imagePath + iconImage, new BMap.Size(25, 25), {anchor: new BMap.Size(6, 21), imageOffset: new BMap.Size(0, 0)});
...
//			}
		...
		//开启编辑 圆，面和线
		/*if((typeof mapmarker.shape.enableEditing == 'function') && GFRAME.enableMarkerEditing) {
			mapmarker.shape.enableEditing();
			
			//覆盖物(圆，面和线)的属性发生变化时触发
			mapmarker.shape.addEventListener("lineupdate",function(e){
				if(typeof markerLineupdate == 'function') {
					markerLineupdate(e.type, e.target, e.currentTarget);
				}
			});
		}*/
	}
...
function onZoomOut() {
	if (13 == GFRAME.addMarkerType) {
		...
		$("#zoomOut").attr("src","zoomout_on.gif");
	}
}
...
function addMarkerPoint(){
	if (1 == GFRAME.addMarkerType) {
		...
		$("#addPoint").attr("src","addpoint_on.gif");
	}
};
...
function drawLineEnd(event, overlay) {
	try {
		var points = overlay.getPath();
		var temp = getLatLngString(points);
		doMapDrawMarker(GFRAME.addMarkerType, temp.lng, temp.lat, "FF0000");
...
function searchVehi() {
	if (4 == GFRAME.addMarkerType) {
		...
		GFRAME.drawingManager.setDrawingMode(BMAP_DRAWING_RECTANGLE);
		GFRAME.drawingManager.open();
		$("#btnSearch").attr("src","search_on.gif");
	}
};
...
//	alert('jingDu'+jingDu+',weiDu'+weiDu+',sw.jingDu'+sw.lng+',sw.weiDu'+sw.lat+',ne.jingDu'+ne.lng+',ne.weiDu'+ne.lat);
	return bounds_.containsPoint(point);
}

var visibleMapEventListener = null;//事件
//地图可视区域发生变化时调用(包含更改缩放级别、拖拽地图)
function visibleMapResize(callback) {
	if(typeof callback == 'function') {
		if(visibleMapEventListener != null) {
			GFRAME.map.removeEventListener('resize', visibleMapEventListener);//地图可视区域大小发生变化时会触发此事件。
			GFRAME.map.removeEventListener('zoomend', visibleMapEventListener);//地图更改缩放级别结束时触发触发此事件。
			GFRAME.map.removeEventListener('moveend', visibleMapEventListener);//停止移动地图时触发
			GFRAME.map.removeEventListener('dragend', visibleMapEventListener);//停止拖拽地图时触发。
		}
		GFRAME.map.addEventListener('resize', callback);//地图可视区域大小发生变化时会触发此事件。
		GFRAME.map.addEventListener('zoomend', callback);//地图更改缩放级别结束时触发触发此事件。
		GFRAME.map.addEventListener('moveend', callback);//停止移动地图时触发。
		GFRAME.map.addEventListener('dragend', callback);//停止拖拽地图时触发。
		visibleMapEventListener = callback;
	}
}
...
function enableMarkerEditing(markerId, enable) {
	...
	if(mapmarker != null) {
		if(enable) {
			if(typeof mapmarker.shape.enableEditing == 'function') {
				mapmarker.shape.enableEditing();
				//覆盖物(圆，面和线)的属性发生变化时触发
				if(typeof markerLineupdate == 'function') {
					mapmarker.shape.addEventListener("lineupdate", markerLineupdate);
				}
			}
		}else {
			if(typeof mapmarker.shape.disableEditing == 'function') {//百度地图
				mapmarker.shape.disableEditing();
				if(typeof markerLineupdate == 'function') {
					mapmarker.shape.removeEventListener("lineupdate", markerLineupdate);
				}
			}
		}
	}
...
/**
 * 判断该点是否在points所在区域范围内
 * @param lng  经度
 * @param lat 纬度
 * @param points  点范围  (Array)
 * @returns {Boolean}
 *//*
function isPtInPoly(lng, lat , points) {
	var flag = true;
	if(points.length < 3) {
		if(points[0].lng >= points[1].lng) {
			if(lng > points[0].lng || lng < points[1].lng) {
				flag = false;
			}
		}else {
			if(lng < points[0].lng || lng > points[1].lng) {
		...
		if(flag) { 
			if(points[0].lat >= points[1].lat) {
				if(lat > points[0].lat || lat < points[1].lat) {
					flag = false;
				}
			}else {
				if(lat < points[0].lat || lat > points[1].lat) {
...
function searchGasStation() {
	if (20 == GFRAME.addMarkerType) {
		GFRAME.resetMarker();
	} else {
		GFRAME.resetDrawMarker();
		GFRAME.addMarkerType = 20;	
		GFRAME.drawingManager.setDrawingMode(BMAP_DRAWING_RECTANGLE);
		GFRAME.drawingManager.open();
	}
};