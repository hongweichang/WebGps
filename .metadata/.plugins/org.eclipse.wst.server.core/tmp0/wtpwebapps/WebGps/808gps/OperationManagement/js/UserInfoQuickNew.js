...
function loadPage(){
	...
	content += '<label for="checkbox-digitalIntercom">' + parent.lang.enable + '</label>';
	content += '</div>';
	$('.td-validity .span-tip').remove();
	$('.td-validity').append(content);
	$('.td-validity .module').click(function() {
		if($('#checkbox-digitalIntercom').attr('checked') == 'checked') {
...
//		}
		...
		if(type == 'add') {
			...
			$('.input-validity').attr('disabled','disabled');
			...
			if(isMine == 1) {
				if(params.company != null) {
					...
					$('#combox-company').attr('disabled','disabled');
		...
		//	$(".input-account").attr('disabled','disabled');
		...
		//	$(".input-name").attr('disabled','disabled');
			...
			if(params.validity == null) {
				$(".input-validity").val(dateCurDateBeginString2());
				$(".input-validity").click(function(){WdatePicker({lang:parent.langWdatePickerCurLoacl(),dateFmt:'yyyy-MM-dd HH:mm:ss'});});
				$('.input-validity').attr('disabled','disabled');
			}else {
				$('.input-validity').val(dateFormat2TimeString(new Date(params.validity)));
				$('#checkbox-digitalIntercom').attr('checked','checked');
			}
			...
			if(isMine == 1) {
				...
				if(params.role != null){
					...
					$('#combox-role').attr('disabled','disabled');
...
//									for(var i = userRoles.length - 1; i >= 0 ; i--) {
//										if(userRoles[i].parentId != company_.parentId) {
//											userRoles.splice(i, 1);
//										}
//									}
//									var rlg = true;
