	...
	if (typeof parent.lang == "undefined") {
		...
		$('#select-handled').flexPanel({
			...
			//	input : {display: '选择状态',width:'60px',value:'',name : 'useType', pid : 'useType', pclass : 'buttom',bgicon : 'true', hide : false},
				combox: {name : 'handleStatus', option : '2&'+parent.lang.all+'|0&'+parent.lang.track_idle+'|1&'+parent.lang.track_parkEvent}
			}	
		});
		...
		$("#parkDetailTable").flexigrid({
			url: "StandardReportParkAction_detail.action",
			dataType: 'json',
			colModel : [
				...
				{display: parent.lang.begintime, name : 'armTimeStart', width : 120, sortable : false, align: 'center', hide: false},
				{display: parent.lang.endtime, name : 'armTimeEnd', width : 120, sortable : false, align: 'center'},
				{display: parent.lang.report_time, name : 'timeLength', width : 120, sortable : false, align: 'center'},
				{display: parent.lang.report_park_position, name : 'startPosition', width : 250, sortable : false, align: 'center'}
...
function fillCellInfo(p, row, idx, index) {
	...
	if(name == 'plateType') {
		...
		if(row['armInfo'] == 0){
			pos = parent.lang.track_idle;
		}else{
			pos = parent.lang.track_parkEvent;
		}
	...
	}else if(name == 'timeLength') {
		pos = getTimeDifference(row[name]/1000);
	}else {
...
function exportReport(exportType) {
	...
	document.reportForm.action = "StandardReportParkAction_detailExcel.action?toMap="+toMap+"&exportType="+exportType+"&begintime="+$('#begintime').val()+"&endtime="+$('#endtime').val();
