...
function loadDiskerrorDetailPage() {
	...
	if (typeof parent.lang == "undefined") {
		...
		$("#diskErrorDetailTable").flexigrid({
			url: "StandardReportHardwareStatusAction_diskErrorDetail.action",
			dataType: 'json',
			colModel : [
...
function fixHeight() {
	$('#diskErrorDetailTable').flexFixHeight();
}

function loadDiskerrorDetailLang(){
	searchOpt.loadLang();
	$("#reportTitle").text(parent.lang.report_storage_navAlarmDiskerror);
}
...
function exportReport(exportType) {
	...
	document.reportForm.action = "StandardReportHardwareStatusAction_detailExcel.action?type=diskError&toMap="+toMap+"&exportType="+exportType+"&begintime="+$('#begintime').val()+"&endtime="+$('#endtime').val();
