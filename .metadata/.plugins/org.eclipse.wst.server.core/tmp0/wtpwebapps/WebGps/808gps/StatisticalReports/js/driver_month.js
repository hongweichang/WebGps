	...
	if (typeof parent.lang == "undefined") {
		...
		$("#fenceSummaryTable").flexigrid({
			...
			colModel : [
				...
				{display: parent.lang.rule_line, name : 'dn', width : 120, sortable : false, align: 'center'},
...
function loadSuccess() {
	...
	if(summaryRanks){
		for (var i = 0; i < summaryRanks.length; i++) {
			a3.push(getArrayName(parent.driverList, summaryRanks[i].did) + " " + summaryRanks[i].dt.substring(0,7));
...
function generateSummaryRank(){
	...
	title_2=parent.lang.driver_analysis_chart
	title_=parent.lang.before + summaryRanks.length + parent.lang.driver_violation
	...
	chart = new Highcharts.Chart( {
	        ...
	        series: [{
	            ...
	            name: parent.lang.ranking_driver,
	            color: '#89A54E',
	            type: 'spline',
	            data: a1,
	            dataLabels: {
...
function loadFenceSummaryLang(){
	searchOpt.loadLang();
	$("#reportTitle").text(parent.lang.report_trip_driver_month);
	$("#labelSelectVehicle").text(parent.lang.report_select_line);
}
...
function exportReport(exportType) {
	...
	document.reportForm.action = "StandardReportLineAction_summaryExcel.action?exportType="+exportType+"&begintime="+$('#begintime').val()+"&endtime="+$('#endtime').val()+"&type=driver&status=monthly";
