...
function loadFenceSummaryPage() {
	...
	if (typeof parent.lang == "undefined") {
		...
		$('#combox-vehiIdnos').val(parent.lang.report_all_line);
		$('#hidden-vehiIdnos').val(getNewArrayByArray(parent.vehiLineList,'id').toString());
		//加载语言
...
function loadSuccess() {
	...
	if(summaryRanks){
		for (var i = 0; i < summaryRanks.length; i++) {
			a3.push(getArrayName(parent.vehiGroupList, summaryRanks[i].lid) + " " + summaryRanks[i].dt);
...
function generateSummaryRank(){
	...
	title_2=parent.lang.line_daily_analysis_chart
	title_=parent.lang.before + summaryRanks.length + parent.lang.line_daily_violation
	...
	chart = new Highcharts.Chart( {
	        ...
	        series: [{
	            ...
	            name: parent.lang.ranking_daily_line,
	            color: '#89A54E',
	            type: 'spline',
	            data: a1,
	            dataLabels: {
...
function loadFenceSummaryLang(){
	searchOpt.loadLang();
	$("#reportTitle").text(parent.lang.report_trip_line_daily);
...
function exportReport(exportType) {
	...
	document.reportForm.action = "StandardReportLineAction_summaryExcel.action?exportType="+exportType+"&begintime="+$('#begintime').val()+"&endtime="+$('#endtime').val()+"&type=line&status=daily";
...
//选择公司
function selCompany() {
	$.dialog({id:'info', title:parent.lang.report_line_select,content: 'url:StatisticalReports/selectInfo.html?type=selLine&singleSelect=false&selectAll=true&level=0',
