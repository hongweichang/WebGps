...
function ajaxLoadDownStation() {
	...
	//向服务器发送ajax请求
	$.myajax.jsonGet("SysServerAction_allStation.action", function(json,action,success){
		if (success) {
...
function ajaxLoadInfo() {
	...
	$.myajax.showLoading(true, parent.lang.loading, this);
	//向服务器发送ajax请求
	$.myajax.jsonGet("SysServerAction_get.action?idno=" + idno, function(json,action,success){
		if (success) {
			$("#name").val(json.name);
			$("#idno").val(json.idno);
...
function ajaxSaveServer() {
	...
	var action;
	if (isEditServer()) {
		action = 'SysServerAction_save.action?idno=' + getUrlParameter("idno");
	} else {
		action = 'SysServerAction_add.action?svrtype=' + getUrlParameter("svrtype");
	}
