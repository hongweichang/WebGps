...
function loadLang(){
	$("#btnSearchDevice").text(parent.lang.search);
	...
	$("#thName").text(parent.lang.device_name);
	$("#thIDNO").text(parent.lang.IDNO);
	$("#thType").text(parent.lang.device_type);
	$("#thClient").text(parent.lang.device_client);
	if (isAddRelation()) {
		$("#spanRemove").text(parent.lang.addSel);
	} else {
		$("#spanRemove").text(parent.lang.delSel);
	}	
}

function disableForm(disable) {
//	diableInput("#terminalType", disable, true);
//	diableInput("#relationName", disable, true);
//	diableInput("#allName", disable, true);
//	diableInput("#hrefAdd", disable, true);
...
function ajaxRelationList(page, devName) {
	...
	var action = "SysServerAction_storeRelList.action?idno=" + idno;
...
function delRelation(idno) {
	...
	$.myajax.jsonGet("SysServerAction_storeRelDel.action?svrIdno=" + getUrlParameter("idno") + "&devIdno=" + idno, function(json,action,success){
		$.myajax.showLoading(false);
...
function delSelRelation() {
	...
	if (devices.length <= 0) {
		...
		if (isAddRelation()) {
			...
			$.myajax.jsonGet("SysServerAction_storeRelAdd.action?svrIdno=" + getUrlParameter("idno") + "&devIdno=" + devices
			...
			//执行删除操作
			$.myajax.showLoading(true, parent.lang.deleting);
			$.myajax.jsonGet("SysServerAction_storeRelDel.action?svrIdno=" + getUrlParameter("idno") + "&devIdno=" + devices
			, function(json,action,success){
				$.myajax.showLoading(false);
				var curpage = $("#pagination").find("#hideCurrentPage").text();
				ajaxRelationList(curpage);
...
function ajaxRelFreeList(page, devName) {
	...
	var action = "SysServerAction_storeRelFree.action?idno=" + idno;
	if (typeof devName !== "undefined" && devName !== "") {
		action += ("&devName=" + encodeURIComponent(devName));
	}
	var pagination={currentPage:temp, pageRecords:10};
...
function addRelation(idno) {	
	//显示的消息
	$.myajax.showLoading(true, parent.lang.adding);
	$.myajax.jsonGet("SysServerAction_storeRelAdd.action?svrIdno=" + getUrlParameter("idno") + "&devIdno=" + idno, function(json,action,success){
		$.myajax.showLoading(false);
		if (success) {
			alert(parent.lang.addSuc);
			var curpage = $("#pagination").find("#hideCurrentPage").text();
