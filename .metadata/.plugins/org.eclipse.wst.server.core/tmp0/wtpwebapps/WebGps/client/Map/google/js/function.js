...
function parseMarkerPoint(typeId, jindu, weidu) {		//解析标记点信息
	if (typeId == 1 || typeId == 10) {	//点
		...
		if (typeId == 2) {
			...
			bottomPoint = new mapobject.maps.LatLng(latList[0], lngList[0]);
			centerPoint = new mapobject.maps.LatLng(latList[0], lngList[0]);
		}
	...
	if (mapmarker != null) {
		...
		if (mapmarker.nameMarker == null) {
			if (typeId == 2){	//Rectange
				...
				mapmarker.shape = new mapobject.maps.Polyline({
		    			path: posOption.coordinates,   
		      			strokeColor: "#"+color,   
	        			strokeOpacity: "0.8",   
	        			strokeWeight: 3,   
...
function addMarkerPoint(){
	if (1 == GFRAME.addMarkerType) {
		...
		document.getElementById("addPoint").src = "./image/addpoint_on.gif";
	}
};
...
function addMarkerRectangle(){
	if (2 == GFRAME.addMarkerType) {
		...
		GFRAME.showMarkerTip = true;
		document.getElementById("addRectangle").src = "./image/addrectangle_on.gif";
	}
};
...
function addMarkerPolygon() {
	if (3 == GFRAME.addMarkerType) {
		...
		GFRAME.showMarkerTip = true;
		document.getElementById("addPolygon").src = "./image/addpolygon_on.gif";
	}
};
...
function addMarkerLine() {
	if (9 == GFRAME.addMarkerType) {
		...
		GFRAME.showMarkerTip = true;
		document.getElementById("addLine").src = "./image/addline_on.gif";
	}
}
...
function searchVehi() {
	if (4 == GFRAME.addMarkerType) {
		...
		GFRAME.showMarkerTip = true;
		document.getElementById("btnSearch").src = "./image/search_on.gif";
	}
};
