...
function loadLang(){
	$("#labelUser").text(parent.lang.usermgr_permit_labelUser);
...
function ajaxLoadInfo() {
	...
	//向服务器发送ajax请求
	$.myajax.jsonGet("UserAction_getPermit.action?id=" + id, function(json,action,success){
		if (success) {
...
function ajaxSavePermit() {
	...
	var action = 'UserAction_savePermit.action?id=' + getUrlParameter("id");
