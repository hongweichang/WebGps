$(document).ready(function(){
	setTimeout(loadFatigueDetailPage, 50);
});
...
function loadFatigueDetailPage() {
	//等待父窗口加载完成后，再加载资源信息
	if (typeof parent.lang == "undefined") {
		setTimeout(loadFatigueDetailPage, 50);
	} else {
		//加载语言
		loadFatigueDetailLang();
		//加载搜索选项
		searchOpt.initSearchOption();
		$("#btnQuery").click(queryFatigueDetail);
		$("#btnExport").click(exportFatigueDetail);
		$("#btnExportCsv").click(exportFatigueDetailCsv);
		$("#btnExportPdf").click(exportFatigueDetailPdf);
		
		$("#fatigueDetailTable").flexigrid({
			url: "ReportAlarmAction_fatigueDetail.action",
			dataType: 'json',
			colModel : [
...
//					checkbox: true,
			...
			usepager: true,
			autoload: false,
			title: parent.lang.report_navAlarmFatigue,
			useRp: true,
			rp: 15,
...
function loadFatigueDetailLang(){
	searchOpt.loadLang();
	$("#reportTitle").text(parent.lang.report_navAlarmFatigue);
}
...
function queryFatigueDetail() {
	...
	$('#fatigueDetailTable').flexOptions(
...
function fillCellInfo(p, row, idx, index) {
	...
	if(name == 'devIdno') {
		...
		if (row[name] == 49) {
			pos =parent.lang.report_alarm_fatigue + "  " + parent.lang.report_alarmBegin;
		} else {
			pos =parent.lang.report_alarm_fatigue + "  " + parent.lang.report_alarmEnd;
		}
...
function exportReport(exportType) {
	...
	document.reportForm.action = "ReportAlarmAction_detailExcel.action?type=fatigue&toMap="+toMap+"&exportType="+exportType;
	document.reportForm.submit();
}

function exportFatigueDetail() {
	exportReport(1);
}

function exportFatigueDetailCsv() {
	exportReport(2);
}

function exportFatigueDetailPdf() {
	exportReport(3);
}