$(document).ready(function(){
	setTimeout(loadVideoLostDetailPage, 50);
});
...
function loadVideoLostDetailPage() {
	//等待父窗口加载完成后，再加载资源信息
	if (typeof parent.lang == "undefined") {
		setTimeout(loadVideoLostDetailPage, 50);
	} else {
		//加载语言
		loadVideoLostDetailLang();
		//加载搜索选项
		searchOpt.initSearchOption();
		$("#btnQuery").click(queryVideoLostDetail);
		$("#btnExport").click(exportVideoLostDetail);
		$("#btnExportCsv").click(exportVideoLostDetailCsv);
		$("#btnExportPdf").click(exportVideoLostDetailPdf);
		
		$("#videoLostDetailTable").flexigrid({
			url: "ReportAlarmAction_videoLostDetail.action",
			dataType: 'json',
			colModel : [
				...
				{display: parent.lang.channel, name : 'channel', width : 100, sortable : false, align: 'center', hide: false},
...
//					checkbox: true,
			...
			usepager: true,
			autoload: false,
			title: parent.lang.report_navAlarmVideoLost,
			useRp: true,
			rp: 15,
...
function loadVideoLostDetailLang(){
	searchOpt.loadLang();
	$("#reportTitle").text(parent.lang.report_navAlarmVideoLost);
}
...
function queryVideoLostDetail() {
	...
	$('#videoLostDetailTable').flexOptions(
...
function exportReport(exportType) {
	...
	document.reportForm.action = "ReportAlarmAction_detailExcel.action?type=videoLost&toMap="+toMap+"&exportType="+exportType;
	document.reportForm.submit();
}

function exportVideoLostDetail() {
	exportReport(1);
}

function exportVideoLostDetailCsv() {
	exportReport(2);
}

function exportVideoLostDetailPdf() {
	exportReport(3);
}