$(document).ready(function(){
	setTimeout(loadShakeDetailPage, 50);
});
...
function loadShakeDetailPage() {
	...
	if (typeof parent.lang == "undefined") {
		...
		$("#btnQuery").click(queryShakeDetail);
		$("#btnExport").click(exportShakeDetail);
		$("#btnExportCsv").click(exportShakeDetailCsv);
		$("#btnExportPdf").click(exportShakeDetailPdf);
		
		$("#shakeDetailTable").flexigrid({
			url: "ReportAlarmAction_shakeDetail.action",
			dataType: 'json',
			colModel : [
...
//					checkbox: true,
			...
			usepager: true,
			autoload: false,
			title: parent.lang.report_navAlarmShake,
			useRp: true,
			rp: 15,
...
function loadShakeDetailLang(){
	searchOpt.loadLang();
	$("#reportTitle").text(parent.lang.report_navAlarmShake);
}
...
function exportReport(exportType) {
	...
	document.reportForm.action = "ReportAlarmAction_detailExcel.action?type=shake&toMap="+toMap+"&exportType="+exportType;
	document.reportForm.submit();
}

function exportShakeDetail() {
	exportReport(1);
}

function exportShakeDetailCsv() {
	exportReport(2); 
}

function exportShakeDetailPdf() {
	exportReport(3);
}