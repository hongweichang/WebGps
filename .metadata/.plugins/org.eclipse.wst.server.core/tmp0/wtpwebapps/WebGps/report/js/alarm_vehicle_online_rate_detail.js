...
function loadAlarmSummaryPage() {
	...
	if (typeof parent.lang == "undefined") {
		...
		$("#btnExportx").click(wedxcfg);
	}
}

function loadAlarmSummaryLang(){
	searchOpt.loadLang();
	$("#reportTitle").text(parent.lang.report_navNormalOnlineRateDetail);
	$("#thIndex").text(parent.lang.index);
	$("#thName").text(parent.lang.vehiName);
	$("#thPlateColor").text(parent.lang.vehicle_plateColor);
	$("#thOnlineTime").text(parent.lang.report_online_time);
	$("#thOnlineRate").text(parent.lang.report_online_rate);
}
...
function wedxcfg(){
	...
	disableForm(true);
	//清除之前加载的数据
	$.myajax.cleanTableContent("#onlineDetailTable");
	//显示加载过程
	...
	$("#onlineDetailTable").attr("data-options","rownumbers:true,singleSelect:true,url:'"+action+"',method:'get'");
	...
	if (success) {
		if (!$.isEmptyObject(json.summarys)) {
			...
			$.each(json.summarys, function (i, fn) {
				var row = $("#onlineDetailTableTemplate").clone();
				...
				append2Table("#onlineDetailTable", k, row);
				k = k + 1;
			});
		} 
		$.myajax.showPagination("#onlineDetailPagination");
		json.pagination.id = "#onlineDetailPagination";
		json.pagination.tableId = "#onlineDetailTable";
