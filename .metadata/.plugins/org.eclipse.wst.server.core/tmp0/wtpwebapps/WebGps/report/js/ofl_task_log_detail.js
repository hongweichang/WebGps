...
function loadOflTaskLogDetailPage() {
	...
	if (typeof parent.lang == "undefined") {
		...
		$("#oflTaskLogDetailTable").flexigrid({
			url: "ReportDeviceOflTaskLogAction_daily.action",
			dataType: 'json',
			colModel : [
...
//					checkbox: true,
			...
			usepager: true,
			autoload: false,
			title: parent.lang.report_offlineRecordingEquipmentUpgrade,
			useRp: true,
			rp: 15,
...
function loadOflTaskLogDetailLang(){
	searchOpt.loadLang();
	$("#reportTitle").text(parent.lang.report_offlineRecordingEquipmentUpgrade);
	$("#labelMarker").text(parent.lang.completion);
	$("#labelDevVerNum").text(parent.lang.website_download_version);
}

function disableForm(disable) {
	searchOpt.disableForm(disable);
	diableInput("#devVerNum", disable, true);
	diableInput("#nTaskStatus", disable, true);
}
...
function queryOflTaskLogDetail() {
	...
	$('#oflTaskLogDetailTable').flexOptions(
...
function exportReport(exportType) {
	...
	document.reportForm.action = "ReportDeviceOflTaskLogAction_detailExcel.action?type=oflTaskLog&toMap="+toMap+"&exportType="+exportType;
