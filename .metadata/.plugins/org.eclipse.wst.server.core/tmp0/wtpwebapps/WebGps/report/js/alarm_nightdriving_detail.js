$(document).ready(function(){
	setTimeout(loadNightDetailPage, 50);
});
...
function loadNightDetailPage() {
	...
	if (typeof parent.lang == "undefined") {
		...
		$("#btnQuery").click(queryNightDetail);
		$("#btnExport").click(exportNightDetail);
		$("#btnExportCsv").click(exportNightDetailCsv);
		$("#btnExportPdf").click(exportNightDetailPdf);
		
		$("#nightDetailTable").flexigrid({
			url: "ReportAlarmAction_nightDrivingDetail.action",
			dataType: 'json',
			colModel : [
...
//					checkbox: true,
			...
			usepager: true,
			autoload: false,
			title: parent.lang.report_navAlarmNightDriving,
			useRp: true,
			rp: 15,
...
function loadNightDetailLang(){
	searchOpt.loadLang();
	$("#reportTitle").text(parent.lang.report_navAlarmNightDriving);
}
...
function fillCellInfo(p, row, idx, index) {
	...
	if(name == 'devIdno') {
		...
		if (row[name] == 151) {
			pos =parent.lang.report_alarm_nightDriving + "  " + parent.lang.report_alarmBegin;
		} else {
			pos =parent.lang.report_alarm_nightDriving + "  " + parent.lang.report_alarmEnd;
		}
...
function exportReport(exportType) {
	...
	document.reportForm.action = "ReportAlarmAction_detailExcel.action?type=night&toMap="+toMap+"&exportType="+exportType;
	document.reportForm.submit();
}

function exportNightDetail() {
	exportReport(1);
}

function exportNightDetailCsv() {
	exportReport(2);
}

function exportNightDetailPdf() {
	exportReport(3);
}