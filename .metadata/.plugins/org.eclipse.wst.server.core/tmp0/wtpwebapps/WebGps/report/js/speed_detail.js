...
function loadSpeedDetailPage() {
	...
	if (typeof parent.lang == "undefined") {
		...
		$("#btnExport").click(exportSpeedTrack);
		$("#btnExportCsv").click(exportSpeedTrackCsv);
		$("#btnExportPdf").click(exportSpeedTrackPdf);
		
		$("#speedDetailTable").flexigrid({
			url: "ReportSpeedAction_detail.action",
			dataType: 'json',
			colModel : [
				...
				{display: parent.lang.report_speed + gpsGetLabelSpeedUnit(), name : 'speed', width : 100, sortable : false, align: 'center'},
...
//					checkbox: true,
			...
			usepager: true,
			autoload: false,
			title: parent.lang.report_navSpeedDetail,
			useRp: true,
			rp: 15,
...
function loadSpeedDetailLang(){
	searchOpt.loadLang();
	$("#reportTitle").text(parent.lang.report_navSpeedDetail);
}
...
function exportReport(exportType) {
	...
	document.reportForm.action = "ReportSpeedAction_gpstrackExcel.action?toMap="+toMap+"&exportType="+exportType;
	document.reportForm.submit();
}


function exportSpeedTrack() {
	exportReport(1);
}

function exportSpeedTrackCsv() {
	exportReport(2);
}

function exportSpeedTrackPdf() {
	exportReport(3); 
}