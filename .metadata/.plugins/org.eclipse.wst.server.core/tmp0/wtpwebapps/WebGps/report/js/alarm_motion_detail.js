$(document).ready(function(){
	setTimeout(loadMotionDetailPage, 50);
});
...
function loadMotionDetailPage() {
	//等待父窗口加载完成后，再加载资源信息
	if (typeof parent.lang == "undefined") {
		setTimeout(loadMotionDetailPage, 50);
	} else {
		//加载语言
		loadMotionDetailLang();
		//加载搜索选项
		searchOpt.initSearchOption();
		$("#btnQuery").click(queryMotionDetail);
		$("#btnExport").click(exportMotionDetail);
		$("#btnExportCsv").click(exportMotionDetailCsv);
		$("#btnExportPdf").click(exportMotionDetailPdf);
		
		$("#motionDetailTable").flexigrid({
			url: "ReportAlarmAction_motionDetail.action",
			dataType: 'json',
			colModel : [
...
//					checkbox: true,
			...
			usepager: true,
			autoload: false,
			title: parent.lang.report_navAlarmMotion,
			useRp: true,
			rp: 15,
...
function loadMotionDetailLang(){
	searchOpt.loadLang();
	$("#reportTitle").text(parent.lang.report_navAlarmMotion);
}
...
function exportReport(exportType) {
	...
	document.reportForm.action = "ReportAlarmAction_detailExcel.action?type=motion&toMap="+toMap+"&exportType="+exportType;
	document.reportForm.submit();
}

function exportMotionDetail() {
	exportReport(1);
}

function exportMotionDetailCsv() {
	exportReport(2); 
}

function exportMotionDetailPdf() {
	exportReport(3);
}