$(document).ready(function(){
	setTimeout(loadBoardOpenedDetailPage, 50);
});
...
function loadBoardOpenedDetailPage() {
	//等待父窗口加载完成后，再加载资源信息
	if (typeof parent.lang == "undefined") {
		setTimeout(loadBoardOpenedDetailPage, 50);
	} else {
		//加载语言
		loadBoardOpenedDetailLang();
		//加载搜索选项
		searchOpt.initSearchOption();
		$("#btnQuery").click(queryBoardOpenedDetail);
		$("#btnExport").click(exportBoardOpenedDetail);
		$("#btnExportCsv").click(exportBoardOpenedDetailCsv);
		$("#btnExportPdf").click(exportBoardOpenedDetailPdf);
		
		$("#boardOpenedDetailTable").flexigrid({
			url: "ReportAlarmAction_boardOpenedDetail.action",
			dataType: 'json',
			colModel : [
				...
				{display: parent.lang.report_ioinType, name : 'armType', width : 240, sortable : false, align: 'center'},
...
//					checkbox: true,
			...
			usepager: true,
			autoload: false,
			title: parent.lang.report_navAlarmBoardOpened,
			useRp: true,
			rp: 15,
...
function loadBoardOpenedDetailLang(){
	searchOpt.loadLang();
	$("#reportTitle").text(parent.lang.report_navAlarmBoardOpened);
}
...
function queryBoardOpenedDetail() {
	...
	$('#boardOpenedDetailTable').flexOptions(
...
function exportReport(exportType) {
	...
	document.reportForm.action = "ReportAlarmAction_detailExcel.action?type=boardOpened&toMap="+toMap+"&exportType="+exportType;
	document.reportForm.submit();
}

function exportBoardOpenedDetail() {
	exportReport(1);
}

function exportBoardOpenedDetailCsv() {
	exportReport(2);
}

function exportBoardOpenedDetailPdf() {
	exportReport(3);
}