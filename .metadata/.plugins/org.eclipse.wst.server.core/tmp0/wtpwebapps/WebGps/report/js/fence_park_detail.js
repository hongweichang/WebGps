$(document).ready(function(){
	setTimeout(loadFenceParkDetailPage, 50);
});
...
function loadFenceParkDetailPage() {
	//等待父窗口加载完成后，再加载资源信息
	if (typeof parent.lang == "undefined") {
		setTimeout(loadFenceParkDetailPage, 50);
	} else {
		//加载语言
		loadFenceParkDetailLang();
		//加载搜索选项
		searchOpt.initSearchOption();
		$("#btnQuery").click(queryFenceParkDetail);
		$("#btnExport").click(exportFenceParkDetail);
		$("#btnExportCsv").click(exportFenceParkDetailCsv);
		$("#btnExportPdf").click(exportFenceParkDetailPdf);
		//加载区域信息
		$("#markerList").append("<option value='0' selected>" + parent.lang.all + "</option>");
		ajaxLoadMarker();
		
		$("#fenceParkDetailTable").flexigrid({
			url: "ReportFenceAction_parkDetail.action",
			dataType: 'json',
			colModel : [
				...
				{display: parent.lang.report_fence_parkBeginTime, name : 'armTimeStr', width : 120, sortable : false, align: 'center', hide: false},
				{display: parent.lang.report_positionCurrent, name : 'position', width : 250, sortable : false, align: 'center'},
				{display: parent.lang.report_fence_parkEndTime, name : 'outTime', width : 120, sortable : false, align: 'center', hide: false},
				{display: parent.lang.report_fence_parkTime, name : 'param4', width : 120, sortable : false, align: 'center'}
				],
//			sortname: "devIdno",
//			sortorder: "asc",
...
//					checkbox: true,
			...
			usepager: true,
			autoload: false,
			title: parent.lang.report_navFenceParkDetail,
			useRp: true,
			rp: 15,
...
function loadFenceParkDetailLang(){
	searchOpt.loadLang();
	$("#reportTitle").text(parent.lang.report_navFenceParkDetail);
	$("#labelTime").text(parent.lang.report_fence_labelParkTime);
...
function queryFenceParkDetail() {
	...
	$('#fenceParkDetailTable').flexOptions(
...
function exportReport(exportType) {
	...
	document.reportForm.action = "ReportFenceAction_detailExcel.action?type=park&toMap="+toMap+"&exportType="+exportType;
	document.reportForm.submit();
}

function exportFenceParkDetail() {
	exportReport(1);
}

function exportFenceParkDetailCsv() {
	exportReport(2);
}

function exportFenceParkDetailPdf() {
	exportReport(3); 
}