$(document).ready(function(){
	setTimeout(loadAccDetailPage, 50);
});
...
function loadAccDetailPage() {
	...
	if (typeof parent.lang == "undefined") {
		...
		$("#btnQuery").click(queryAccDetail);
		$("#btnExport").click(exportAccDetail);
		$("#btnExportCsv").click(exportAccDetailCsv);
		$("#btnExportPdf").click(exportAccDetailPdf);
		
		$("#accDetailTable").flexigrid({
			url: "ReportAlarmAction_accDetail.action",
			dataType: 'json',
			colModel : [
...
//					checkbox: true,
			...
			usepager: true,
			autoload: false,
			title: parent.lang.report_navAlarmAcc,
			useRp: true,
			rp: 15,
...
function loadAccDetailLang(){
	searchOpt.loadLang();
	$("#reportTitle").text(parent.lang.report_navAlarmAcc);
}
...
function exportReport(exportType) {
	...
	document.reportForm.action = "ReportAlarmAction_detailExcel.action?type=acc&toMap="+toMap+"&exportType="+exportType;
	document.reportForm.submit();
}

function exportAccDetail() {
	exportReport(1);
}

function exportAccDetailCsv() {
	exportReport(2);
}

function exportAccDetailPdf() {
	exportReport(3);
}