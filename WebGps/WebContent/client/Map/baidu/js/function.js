...
function updateVehicleCluster(vehiIdno, jindu, weidu, count){
//	var vehicle = findVehicle(vehiIdno);
//	if (vehicle != null){
//		var point = new BMap.Point(jindu, weidu);
//		if(vehicle._clusterMarker == null) {
//			vehicle._clusterMarker = new BMapLib.TextIconOverlay(point, count);
//			GFRAME.map.addOverlay(vehicle._clusterMarker);
//		}
//		vehicle._clusterMarker.setPosition(point);
//		vehicle._clusterMarker.setText(count);
//		vehicle._clusterMarker.setZIndex(-1);
//	}
	...
	vehicle.setStatus(jindu, weidu, 0, 0, 0); 
	var autoCenter = false;
	var point = new BMap.Point(vehicle.getJindu(), vehicle.getWeidu());
//	var image = GFRAME.getVehicleGroupImage(count);

		if (vehicle.groupmarker != null){
			vehicle.groupmarker.setMap(null);
			vehicle.groupmarker = null;
		}
		if (vehicle.groupNameMarker != null){
			vehicle.groupNameMarker.setMap(null);
			vehicle.groupNameMarker = null;
		}
		
	if (null == vehicle.groupmarker){
		var vehiText = null;
		var showPop = false;
		...
		var groupmarker = new GroupMarker({position:point,map:GFRAME.map,text:count,id:vehiIdno,showpop:showPop});
		vehicle.groupmarker = groupmarker;
		vehicle.setNameMarkerType = 1;
		var namemarker = new groupNameMarker({position:point,map:GFRAME.map,text:count});
		vehicle.groupNameMarker = namemarker;
	}else{
		var vehiText = count;
		if (vehiIdno == GFRAME.openPopMarkerVehicle){
			vehiText = getTxtByVehicle(vehicle);
		}
		//vehicle.groupmarker.update({position:point,icon:image,text:vehiText});
		vehicle.groupmarker.update({position:point,text:vehiText});
		vehicle.groupNameMarker.update({position:point,text:vehiText});
	}
...
function showVehiclePopEx(vehiIdno) {
	if (false == GFRAME.isLastClickMap) {		
		//if (GFRAME.LastPopVehicleName != null)	{
		//	var vehicle = findVehicle(GFRAME.LastPopVehicleName);
		//	if (vehicle != null) {		
		//		hideVehiclePop();
		//		showVehiclePop(vehicle);
		//		return ;
...
// 居中关注车辆,不关注或在地图内,不操作
function centerVehicleEx(vehiIdno, jingDu, weiDu){	
	if (false == GFRAME.isLastClickMap && vehiIdno == GFRAME.LastPopVehicleName) {
...
function deleteVehicle(vehiIdno){	//删除车辆信息
	...
	if (findVehi != null){
		...
		if(findVehi._clusterMarker != null) {
			GFRAME.map.removeOverlay(findVehi._clusterMarker);
			findVehi._clusterMarker = null;
		}
		if (GFRAME.openPopMarkerVehicle == vehiIdno){
			GFRAME.openPopMarkerVehicle = null;
			GFRAME.vehicleCenter = false;
		}
		if (findVehi.groupmarker != null){
			findVehi.groupmarker.setMap(null);
			findVehi.groupmarker = null;
		}
		if (findVehi.groupNameMarker != null){
			findVehi.groupNameMarker.setMap(null);
			findVehi.groupNameMarker = null;
		}
	...
	if (mapmarker != null) {
		...
		if (mapmarker.nameMarker == null) {
			if (typeId == 1) {
				...
				var pt = new BMap.Point(jindu, weidu);
				mapmarker.shape = new BMap.Circle(pt, parseInt(param, 10), {
		      		strokeColor: "#" + color,   
	        		strokeOpacity: "0.8",   
	        		strokeWeight: 2,   
	        		fillColor: "#" + color,   
...
function trackDrawPoint(trackId) {
	...
	if (track != null){
		...
		if (length > 1){
			...
			var polyLine = new BMap.Polyline(poly, {strokeColor:"#00FF00",strokeOpacity:0.9,strokeWeight:2,enableClicking:false});    	
			GFRAME.map.addOverlay(polyLine);
...
function openRectangleZoom(zoom, tip) {
	...
	GFRAME.rectangleZoom = new BMapLib.RectangleZoom(GFRAME.map, {
			zoomType: zoomVal,
  		followText: tip,
  		autoClose: false
		}, rectangleZoomEnd);
	GFRAME.rectangleZoom.setStrokeColor("#FF0000");
	GFRAME.rectangleZoom.setLineStroke(2);
	GFRAME.rectangleZoom.setFillColor("#FF0000");
	GFRAME.rectangleZoom.setOpacity(0.4);
	GFRAME.rectangleZoom.open();
};

function rectangleZoomEnd() {
	alert("rectangleZoomEnd");
	GFRAME.resetMarker();
};
...
function onZoomIn() {
	if (12 == GFRAME.addMarkerType) {
		...
		openRectangleZoom(true, lang.tipZoomIn);
		document.getElementById("zoomIn").src = GFRAME.imagePath + "zoomin_on.gif";
	}
};
...
function onZoomOut() {
	if (13 == GFRAME.addMarkerType) {
		...
		openRectangleZoom(false, lang.tipZoomOut);
		document.getElementById("zoomOut").src = GFRAME.imagePath + "zoomout_on.gif";
	}
}
...
function onDistance() {
	if (14 == GFRAME.addMarkerType) {
		...
		if (GFRAME.distanceTool == null){
			GFRAME.distanceTool = new BMapLib.DistanceTool(GFRAME.map);
			GFRAME.distanceTool.addEventListener("drawend", distanceEnd);
		}
		GFRAME.resetDrawMarker();
		GFRAME.addMarkerType = 14;
		GFRAME.distanceTool.open();
		document.getElementById("distance").src = GFRAME.imagePath + "distance_on.gif";
	}
};

function distanceEnd(event){ 
	GFRAME.resetMarker();
};
...
function addMarkerPoint(){
	if (1 == GFRAME.addMarkerType) {
		...
		GFRAME.drawingManager.setDrawingMode(BMAP_DRAWING_MARKER);
		GFRAME.drawingManager.open();
		document.getElementById("addPoint").src = GFRAME.imagePath + "addpoint_on.gif";
	}
};

function drawMarkerEnd(event, overlay){ 
	...
	GFRAME.map.removeOverlay(overlay);
	GFRAME.resetMarker();
};
...
function addMarkerCircle() {
	if (10 == GFRAME.addMarkerType) {
		...
		GFRAME.drawingManager.setDrawingMode(BMAP_DRAWING_CIRCLE);
		GFRAME.drawingManager.open();
		document.getElementById("addCircle").src = GFRAME.imagePath + "addcircle_on.gif";
	}
}

function drawCircleEnd(event, overlay) {
	var latlng = overlay.getCenter();
	try {
		if (isChrome) {
			app.sendMessage('OnMapMarker', [GFRAME.addMarkerType, latlng.lng.toFixed(6).toString(), latlng.lat.toFixed(6).toString(), overlay.getRadius().toString()]);
		} else {
			window.external.OnMapMarker(GFRAME.addMarkerType, latlng.lng.toFixed(6).toString(), latlng.lat.toFixed(6).toString(), overlay.getRadius().toString());
		}
	} catch(err) {
	}
	GFRAME.map.removeOverlay(overlay);
...
function addMarkerRectangle(){
	if (2 == GFRAME.addMarkerType) {
		...
		GFRAME.drawingManager.setDrawingMode(BMAP_DRAWING_RECTANGLE);
		GFRAME.drawingManager.open();
		document.getElementById("addRectangle").src = GFRAME.imagePath + "addrectangle_on.gif";
	}
};

function drawRectangleEnd(event, overlay){
	try {
...
//	GFRAME.showMarkerTip = true;
		GFRAME.drawingManager.setDrawingMode(BMAP_DRAWING_POLYGON);
		GFRAME.drawingManager.open();
		document.getElementById("addPolygon").src = GFRAME.imagePath + "addpolygon_on.gif";
	}
};

function drawPolygonEnd(event, overlay) {
	try {
...
function addMarkerLine() {
	if (9 == GFRAME.addMarkerType) {
		...
		GFRAME.drawingManager.setDrawingMode(BMAP_DRAWING_POLYLINE);
		GFRAME.drawingManager.open();
...
function callbackMapBounds(){ 
	var bounds = GFRAME.map.getBounds();
	var SouthWest = bounds.getSouthWest();
	var NorthEast = bounds.getNorthEast();
	try {
		if (isChrome) {
			app.sendMessage('OnGetMapBounds', [SouthWest.lng.toFixed(6).toString(), SouthWest.lat.toFixed(6).toString(), NorthEast.lng.toFixed(6).toString(), NorthEast.lat.toFixed(6).toString()]);
		} else {
			window.external.OnGetMapBounds(SouthWest.lng.toFixed(6).toString(), SouthWest.lat.toFixed(6).toString(), NorthEast.lng.toFixed(6).toString(), NorthEast.lat.toFixed(6).toString());
		}
	} catch(err) {
	}
};
...
function callbackVersions(){ 
	try {
		if (isChrome) {
			app.sendMessage('OnGetVersions', ['20160307']);
		} else {
			window.external.OnGetVersions('20160307');
		}
	} catch(err) {
	}
};

function ClickMap(vehicle){ 
		setTimeout(function () {
		callbackClickMap(vehicle);	}, 0);
}

function callbackClickMap(vehicle){ 
	try {
		if (vehicle != null){
			if (isChrome) {
				app.sendMessage('OnClickMap', [GFRAME.MapGlobalID, vehicle.getIdno()]);
			} else {
				window.external.OnClickMap(GFRAME.MapGlobalID, vehicle.getIdno());
			}			
		}
		else {			
			if (isChrome) {
				app.sendMessage('OnClickMap', [GFRAME.MapGlobalID, '-1']);
			} else {
				window.external.OnClickMap(GFRAME.MapGlobalID, '-1');
			}	
		}			
	} catch(err) {
	}
...
function SetMapGlobalID(nID) {
	GFRAME.MapGlobalID = nID;
}
