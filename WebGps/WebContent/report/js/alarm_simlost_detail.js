...
function loadSinnalDetailPage() {
	...
	if (typeof parent.lang == "undefined") {
		...
		$("#gpssinnalDetailTable").flexigrid({
			url: "ReportAlarmAction_simLostDetail.action",
			dataType: 'json',
			colModel : [
...
//					checkbox: true,
			...
			usepager: true,
			autoload: false,
			title: parent.lang.report_navSimLost,
			useRp: true,
			rp: 15,
...
function loadSinnalDetailLang(){
	searchOpt.loadLang();
	$("#reportTitle").text(parent.lang.report_navSimLost);
}
...
function exportReport(exportType) {
	...
	document.reportForm.action = "ReportAlarmAction_detailExcel.action?type=simlost&toMap="+toMap+"&exportType="+exportType;
