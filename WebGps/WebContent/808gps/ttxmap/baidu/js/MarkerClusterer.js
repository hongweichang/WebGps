...
function MarkerClusterer(map, options) {
    ...
    this._maxZoom =  Number(opts["maxZoom"]) || 18;
    ...
    //事件
    this.zoomendEventListener = null;
    this.moveendEventListener = null;
    //初始化点聚合事件
...
MarkerClusterer.prototype.initEventListener = function(){
	var that = this;
	this.zoomendEventListener = function() {
		 that._redraw();
	};
	this.moveendEventListener = function() {
		 that._moveendRedraw();
	};
	
    this._map.addEventListener("zoomend", this.zoomendEventListener);
    this._map.addEventListener("moveend", this.moveendEventListener);
};
...
MarkerClusterer.prototype.removeEventListener = function(){
	if(this.zoomendEventListener != null) {
		this._map.removeEventListener('zoomend', this.zoomendEventListener);
	}
	if(this.moveendEventListener != null) {
		this._map.removeEventListener('moveend', this.moveendEventListener);
	}
};
...
MarkerClusterer.prototype._createClusters = function(){
    ...
    for(var i = 0, vehiIdno; vehiIdno = this._vehiIdnos[i]; i++){
    	var vehicle = GFRAME.vehicleList.get(vehiIdno);
    	if(vehicle != null && !vehicle.isInCluster && extendedBounds.containsPoint(vehicle.getPosition())) {
...
MarkerClusterer.prototype._addToClosestCluster = function (vehiIdno){
    ...
    for(var i = 0, cluster; cluster = this._clusters[i]; i++){
        ...
        if(center){
            var d = this._map.getDistance(center, vehicle.getPosition());
            if(d < distance){
                distance = d;
...
function Cluster(markerClusterer){
    ...
    this._clusterMarker = new BMapLib.TextIconOverlay(this._center, this._vehiIdnos.length, {"styles":this._markerClusterer.getStyles()});
...
Cluster.prototype.isMarkerInClusterBounds = function(vehicle) {
    return this._gridBounds.containsPoint(vehicle.getPosition());
};
	...
	if(vehicle != null) {
		if(vehicle.popMarker) {
			if(useDeleteVehicle) {
				...
				//未初始化
				if(!vehicle.popMarker.isInitialize) {
					vehicle.popMarker.setMap(this._map);
				}else {
		...
		if(vehicle.nameMarker) {
			if(useDeleteVehicle) {
				...
				//未初始化
				if(!vehicle.nameMarker.isInitialize) {
					vehicle.nameMarker.setMap(this._map);
				}else {
