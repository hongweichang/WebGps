...
PopupMarker.prototype = new BMap.Overlay();//new google.maps.OverlayView();
...
PopupMarker.prototype.setMap = function(nativeMap){
	if(nativeMap==null)
...
//  this.map_.getContainer().appendChild(dummyTextNode);
  ...
  /*if(totalSize.width > 400 * lines.length) {
	  var ta = Math.ceil(totalSize.width/(400 * lines.length));
	  totalSize.width = 400 * lines.length;
	  totalSize.height += (ta - 1) * 24; 
  }*/
...
PopupMarker.prototype.showIconMarker = function(){
	if (this.iconContainer) {
		this.iconContainer.style.visibility = "visible";
	}
};