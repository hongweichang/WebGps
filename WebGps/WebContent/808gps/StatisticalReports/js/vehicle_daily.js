...
function loadSuccess() {
	...
	if(summaryRanks){
		for (var i = 0; i < summaryRanks.length; i++) {
			a3.push(getArrayName(parent.vehicleList, summaryRanks[i].vn) + " " + summaryRanks[i].dt);
...
function generateSummaryRank(){
	...
	title_2=parent.lang.vehicle_daily_analysis_chart
	title_=parent.lang.before + summaryRanks.length + parent.lang.vehicle_daily_violation
	...
	chart = new Highcharts.Chart( {
	        ...
	        series: [{
	            ...
	            name: parent.lang.ranking_daily_vehicle,
	            color: '#89A54E',
	            type: 'spline',
	            data: a1,
	            dataLabels: {
...
function loadFenceSummaryLang(){
	searchOpt.loadLang();
	$("#reportTitle").text(parent.lang.report_trip_vehicle_daily);
}
...
function exportReport(exportType) {
	...
	document.reportForm.action = "StandardReportLineAction_summaryExcel.action?exportType="+exportType+"&begintime="+$('#begintime').val()+"&endtime="+$('#endtime').val()+"&type=vehi&status=daily";
