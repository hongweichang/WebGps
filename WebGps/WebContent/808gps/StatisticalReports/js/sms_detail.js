$(document).ready(function(){
	setTimeout(loadSMSDetailPage, 50);
});

var searchOpt = new searchOption(false, true);

function loadSMSDetailPage() {
	//等待父窗口加载完成后，再加载资源信息
	if (typeof parent.lang == "undefined") {
		...
		$(".btnQuery").click(querySMSDetail);
		$(".btnExport").click(exportSMSDetail);
		$(".btnExportCSV").click(exportSMSDetailCSV);
		$(".btnExportPDF").click(exportSMSDetailPDF);
		var width = 0;
		...
		$("#smsDetailTable").flexigrid({
			url: "StandardUserLogAction_smsLogDetail.action",
			dataType: 'json',
			colModel : [
				{display: parent.lang.index, name : 'index', width : 40, sortable : false, align: 'center'},
				{display: parent.lang.report_user, name : 'param3', width : 120, sortable : false, align: 'center'},
				{display: parent.lang.plate_number, name : 'param4', width : 120, sortable : false, align: 'center', hide: false},
				...
				{display: parent.lang.report_command, name : 'param1', width : 400, sortable : false, align: 'center'}
				],
...
function fixHeight() {
	$('#smsDetailTable').flexFixHeight();
}

function loadSMSDetailLang(){
	searchOpt.loadLang();
	$("#reportTitle").text(parent.lang.report_sms_detail);
}
...
function fillCellInfo(p, row, idx, index) {
	...
	if(name == 'plateType') {
		...
		pos = row['vehicle'].vehiColor;
	}else if (name == 'user') {
		pos = row['user'].account;
	}else {
...
function exportReport(exportType) {
	...
	document.reportForm.action = "StandardUserLogAction_detailExcel.action?type=sms&exportType="+exportType+"&begintime="+$('#begintime').val()+"&endtime="+$('#endtime').val();
	document.reportForm.submit(); 
}
//导出至excel，导出至csv，导出至pdf
function exportSMSDetail() {
	exportReport(1);
}

function exportSMSDetailCSV() {
	exportReport(2);
}

function exportSMSDetailPDF() {
	exportReport(3);
}
