...
function loadOfflineUpgradeDetailPage() {
	...
	if (typeof parent.lang == "undefined") {
		...
		$(".btnQuery").click(queryVehicleVideosDetail);
...
function loadOfflineUpgradeDetailLang(){
	searchOpt.loadLang();
	$("#reportTitle").text(parent.lang.report_vehicle_video);
	$("#labelCompletion").text(parent.lang.mediaStatus);
}

function queryVehicleVideosDetail() {
	...
	var action = "StandardReportMediaAction_videoList.action";
...
function dis(index,photo) {
	...
	content += '	<img src="image/icon3.jpg" class="photo-img" alt="" id=img'+ index +'  />';
	...
	content += '<form method="post" action="StandardReportMediaAction_getMedia.action?filePath='+photo.filePath+'&fileSize='+photo.fileSize+'"><input type="submit" style="margin-left: 5px" value="' + parent.lang.download + '"/></form>';content += '</div>';
	$('#vehicleVideos').append(content);
}