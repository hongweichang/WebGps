...
function loadFenceSummaryPage() {
	...
	if (typeof parent.lang == "undefined") {
		...
		$('#selectName').flexPanel({
			InputModel : {display: parent.lang.selectDriverTip,value:'',name : 'vehiIdnos', pid : 'vehiIdnos', pclass : 'buttom',bgicon : '',hidden:true, hide : false} 
		});
		...
		$('#selectName').flexPanel({
			InputModel : {display: parent.lang.selectLineTip,value:'',name : 'vehiIdnos', pid : 'vehiIdnos', pclass : 'buttom',bgicon : '',hidden:true, hide : false} 
		});
		$('#combox-vehiIdnos').val(parent.lang.report_all_driver);
		$('#hidden-vehiIdnos').val(getNewArrayByArray(parent.driverList,'id').toString());
		//加载语言
...
function loadSuccess() {
	...
	if(summaryRanks){
		for (var i = 0; i < summaryRanks.length; i++) {
			a3.push(getArrayName(parent.driverList, summaryRanks[i].did) + " " + summaryRanks[i].dt);
...
function generateSummaryRank(){
	...
	title_2=parent.lang.driver_daily_analysis_chart
	title_=parent.lang.before + summaryRanks.length + parent.lang.driver_daily_violation
	...
	chart = new Highcharts.Chart( {
	        ...
	        series: [{
	            ...
	            name: parent.lang.ranking_daily_driver,
	            color: '#89A54E',
	            type: 'spline',
	            data: a1,
	            dataLabels: {
...
function loadFenceSummaryLang(){
	searchOpt.loadLang();
	$("#reportTitle").text(parent.lang.report_trip_driver_daily);
	$("#labelSelectVehicle").text(parent.lang.report_select_driver);
}
...
function exportReport(exportType) {
	...
	document.reportForm.action = "StandardReportLineAction_summaryExcel.action?exportType="+exportType+"&begintime="+$('#begintime').val()+"&endtime="+$('#endtime').val()+"&type=driver&status=daily";
...
//选择公司
function selCompany() {
	$.dialog({id:'info', title:parent.lang.report_driver_select,content: 'url:StatisticalReports/selectInfo.html?type=selDriver&singleSelect=false&selectAll=true',
