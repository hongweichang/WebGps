$(document).ready(function(){
	setTimeout(loadOnlineRateDetailPage, 50);
});

var searchOpt = new searchOption(false,true);

function loadOnlineRateDetailPage() {
	//等待父窗口加载完成后，再加载资源信息
	if (typeof parent.lang == "undefined") {
		...
		//加载语言
		loadOnlineRateDetailLang();
		//加载搜索选项
		searchOpt.initSearchOption();
		$('#combox-vehiIdnos').on('click keyup',selVehicle);
		$(".btnQuery").click(queryOnlineRateDetail);
		$(".btnExport").click(exportOnlineRateDetail);
		$(".btnExportCSV").click(exportOnlineRateDetailCSV);
		$(".btnExportPDF").click(exportOnlineRateDetailPDF);
		var width = 0;
		...
		$("#onlineRateDetailTable").flexigrid({
			...
			colModel : [
					...
					{display: parent.lang.report_online_rate, name : 'param1SumStr', width : 100, sortable : false, align: 'center'}
				],
...
function fixHeight() {
	$('#onlineRateDetailTable').flexFixHeight();
}

function loadOnlineRateDetailLang(){
	searchOpt.loadLang();
	$("#reportTitle").text(parent.lang.report_online_rate_detail);
}
...
function queryOnlineRateDetail() {
	...
	$('#onlineRateDetailTable').flexOptions(
...
function exportReport(exportType) {
	...
	document.reportForm.action = "StandardReportLoginAction_summaryExcel.action?type=rate&exportType="+exportType+"&begintime="+$('#begintime').val()+"&endtime="+$('#endtime').val();
	document.reportForm.submit(); 
}
//导出至excel，导出至csv，导出至pdf
function exportOnlineRateDetail() {
	exportReport(1);
}

function exportOnlineRateDetailCSV() {
	exportReport(2);
}

function exportOnlineRateDetailPDF() {
	exportReport(3);
}
