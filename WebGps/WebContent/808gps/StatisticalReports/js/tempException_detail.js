...
function loadAlarmDetailPage() {
	...
	if (typeof parent.lang == "undefined") {
		...
		$("#alarmDetailTable").flexigrid({
			url: "StandardReportTempAction_detail.action",
			dataType: 'json',
			colModel : [
				...
				{display: parent.lang.report_probe_no, name : 'alarmSource', width : 60, sortable : false, align: 'center'},
...
function loadAlarmDetailLang(){
	searchOpt.loadLang();
	$("#reportTitle").text(parent.lang.report_temp_detailException);
...
function exportReport(exportType) {
	...
	document.reportForm.action = "StandardReportTempAction_detailExcel.action?exportType="+exportType+"&toMap="+toMap+"&begintime="+$('#begintime').val()+"&endtime="+$('#endtime').val();
...
function getArmTypes() {
	var armType = [];
	armType.push({id:2,name:parent.lang.all});
	armType.push({id:0,name:parent.lang.report_high_temperature_alarm});
	armType.push({id:1,name:parent.lang.report_low_temperature_alarm});
	return armType;
}